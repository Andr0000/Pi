...
int main()
{
	...
	string decmj = to_string(dp);
	time(&trenutno_vrijeme);
	string vremenska_oznaka = to_string( (int)trenutno_vrijeme ); //vremenski marker za ime datoteke
	ofstream moja_datoteka;
	moja_datoteka.open ("Pi_broj_" + decmj + "DP_" +  vremenska_oznaka + ".txt");//kreiranje datoteke
    ...
    thread nit1 ( arctg, ref(suma01), 8, dp, br_cl1 );//kreiranje niti
    thread nit2 (arctg, ref(suma02), 57, dp, br_cl2 );
    thread nit3 (arctg, ref(suma03), 239, dp, br_cl3 );
    thread nit7 ( arctg, ref(suma01ctrl), 18, dp, br_cl1_1 );//kreiranje niti
    thread nit8 (arctg, ref(suma02ctrl), 57, dp, br_cl2 );
    thread nit9 (arctg, ref(suma03ctrl), 239, dp, br_cl3 );  
    nit7.join();//pauzira do zavrsetka niti
    nit8.join();   
    nit9.join();    
    nit1.join();//pauzira do zavrsetka niti
    nit2.join();   
    nit3.join();
    thread nit4 ( mnozenje, ref(suma01), 24);
    thread nit5 ( mnozenje, ref(suma02), 8);
    thread nit6 ( mnozenje, ref(suma03), 4);
    thread nit10 ( mnozenje, ref(suma01ctrl), 48);
    thread nit11 ( mnozenje, ref(suma02ctrl), 32);
    thread nit12 ( mnozenje, ref(suma03ctrl), 20);
    nit10.join();
    nit11.join();
    nit12.join();    
    nit4.join();
...
//    ispis ( suma03ctrl );
    ...
    int i=0;    
    while ( suma03[i]==suma03ctrl[i] && suma03[i+1]==suma03ctrl[i+1] ){
            if ( i==0 ){
                moja_datoteka<<suma03[i]<<".\n";
            }
            else{
                moja_datoteka<<suma03[i];
                if ( (i%10)==0 )
